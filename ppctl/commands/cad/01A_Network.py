...
def __do_action():
	if __curr_index == 0:
		...
		__cad.lcd.clear()
		if __orig_screen != "" and __orig_listener is not None:
			__cad.lcd.write(__orig_screen)
			__orig_listener.activate()
...
def start_module(cad, listener, screen):
	global __cad
	global __orig_listener
	global __orig_screen
	__cad = cad
	__orig_listener = listener
	__orig_screen = screen
	__cad.lcd.clear()
	__cad.lcd.blink_off()
